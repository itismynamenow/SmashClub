                            ...
                            for (int l = 0; l < jump_height; l++)
                            {
                                if (tileMap[ClampRow(y + l), ClampColumn(x + k)] == 1)
                                {
                                                                 ...
                                                                 new Node(ClampRow(y - 1), ClampColumn(x), "jump_start")))
                                    {
                                        ...
                                        graph[currentJumStartNodeID].neighbors.Add(graph[currentFallNodeID]);
                                        graph[currentFallNodeID].neighbors.Add(graph[currentJumStartNodeID]);
...
//**********************************************************************************************************************************************************************************************//
        ...
        {            
            ...
            for (int j=1;j<tileMap.GetLength(1);j++)
            {
                   ...
                   tileMap[ClampRow(graph[i].row), ClampColumn(graph[i].column - j)] != 1)
                {
                    ...
                    {
                        graph[i].neighbors.Add(graph[nodeMap[ClampRow(graph[i].row), ClampColumn(graph[i].column - j)][0]]);
                        break;
                    }
                }
        ...
        for (int i = 0; i < graph.Count; i++)
        {
            ...
            {
                PrintLinksToNeighborsRecursivly(graph[i],1);
            }
        }
        //PrintGraph();
        //PrintLinksToNeighbors("jump_start");
    ...
    void CopyNeighbors(ref List<Node> graph, int copyfromNodeID,int copyToNodeID)
    {
        for(int i=0;i<graph[copyfromNodeID].neighbors.Count;i++)
        {
            ...
            {
                if(graph[copyfromNodeID].neighbors[i].column == graph[copyToNodeID].neighbors[j].column &&
                   graph[copyfromNodeID].neighbors[i].row == graph[copyToNodeID].neighbors[j].row &&
                   graph[copyfromNodeID].neighbors[i].type.Equals( graph[copyToNodeID].neighbors[j].type))
                {
                    hasCopy = true;
                    break;
                }
    ...
    void AddNeighbor(ref List<Node> graph,int node, int neighbor)
    {
        ...
        else
        {
            for(int i=0;i< graph[node].neighbors.Count;i++)
            {
                ...
                if(graph[node].neighbors[i].ID == graph[neighbor].ID)
                {
                    return;
                }
            }
